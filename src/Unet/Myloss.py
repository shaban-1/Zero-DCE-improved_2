...
class SupervisedLoss(nn.Module):
    ...
    def forward(self, enhanced_image, target_image):
        return self.l1_loss(enhanced_image, target_image)
...
class CustomLoss(nn.Module):
    def __init__(self):
        ...
        # Создаем экземпляр SupervisedLoss один раз в __init__
        self.supervised_loss = SupervisedLoss()  # Переименовал для консистентности

    def forward(self, enhanced_image, target_image, img_lowlight, A):
        ...
        # Вычисляем SupervisedLoss с использованием enhanced_image и target_image
        loss_super = self.supervised_loss(enhanced_image, target_image)
        # Суммируем все лоссы
        loss = Loss_TV + loss_spa + loss_exp + loss_sa + loss_cc + loss_percept + loss_super
        return loss
