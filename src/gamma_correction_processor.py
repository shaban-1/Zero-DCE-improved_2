...
def process_images(input_dir):
    ...
    filenames = [filename for filename in os.listdir(input_dir) if benign_pattern.match(filename)]

    # Используем tqdm для отображения прогресса
    for filename in tqdm(filenames, desc="Обработка изображений", unit="image"):
...
def move_images_to_folders(input_folder, dark_images_folder, light_images_folder):
    ...
    # Используем tqdm для отображения прогресса
    for filename in tqdm(filenames, desc="Перемещение изображений", unit="image"):
